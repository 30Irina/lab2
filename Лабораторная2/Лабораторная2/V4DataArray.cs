...
public class V4DataArray : V4Data, IEnumerable<DataItem>
{
        ...
        for (int i = 0; i < this.Ox; i++)
            for (int j = 0; j < this.Oy; j++)
            {
                ...
                res += "значение = " + String.Format(format, matrix[i, j]) + "\n" +
                    " значение модуля поля = " + String.Format(format, System.Numerics.Vector2.Distance(System.Numerics.Vector2.Zero, matrix[i,j])) + "\n";
    ...
    public override IEnumerator<DataItem> GetEnumerator()
    {
        for (int i = 0; i < Ox; i++)
            for (int j = 0; j < Oy; j++)
            {
                var point = new Vector2(i * this.step.X, j * this.step.Y);
